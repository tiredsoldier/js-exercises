...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    return restaurants
      .filter(
        array => array.totalSeats - array.numberOfCustomers >= numberOfPeople
      )
  ...
  findRestaurantServingDish: function(dishName) {
    return restaurants
      .filter(array => array.menu.indexOf(dishName) >= 0)
      .map(array => array.name);
  },
  // Complete here
  countNumberOfRestaurantsInArea: function(area) {
    return restaurants.filter(array => array.address.area === area).length;
    // Complete here
  }
};
