...
var destinationNameReachableByFerry = travelDestinations
  .filter(name => name.transportations.indexOf("ferry") >= 0)
  .map(name => name.destinationName); // Complete here

var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(
    name => name.transportations.indexOf("train") >= 0 && name.distanceKms > 300
  )
  .map(name => name.destinationName); // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
